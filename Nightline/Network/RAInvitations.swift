...
class RAInvitations: RABase {
  ...
  func acceptGroupInvitation(invitationID: String, callback: @escaping (responseServer) -> ()) {
    let url = RoutesAPI.baseUrl + "/invitations/groups/\(invitationID)/accept"
    Alamofire.request(url).responseJSON(completionHandler: {
      response in
      switch response.result {
      case .failure(let error):
  ...
  func declineGroupInvitation(invitationID: String, callback: @escaping (responseServer) -> ()) {
    let url = RoutesAPI.baseUrl + "/invitations/groups/\(invitationID)/decline"
    Alamofire.request(url).responseJSON(completionHandler: {
      response in
      switch response.result {
      case .failure(let error):
